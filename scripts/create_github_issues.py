...
def extract_issues(content: str) -> list[dict]:
    ...
    # Match "# Issue N" then any following text (handles encoding issues with dashes)
    issue_pattern = r'^#\s+Issue\s+(\d+[a-z]?)\s+(.+)$'
    issues = []
    
    # Find all issue blocks using regex
    current_issue = None
    current_block = []
    issue_number = None
    
    for line in content.split('\n'):
        # Check if this is an issue header (match more permissively)
...
def parse_issue_details(issue: dict) -> dict:
    ...
    return {
        'issue_number': issue['issue_number'],  # Use the number captured during extraction
        'title': title,
...
def main():
    ...
    print(f"Reading issues from MVP_GITHUB_ISSUES_COMPLETE.md...")
    ...
    if dry_run:
        print(f"\n{Colors.CYAN}Dry run complete. Run without --dry-run to create issues.{Colors.END}")
    else:
        print(f"\n{Colors.GREEN}All issues created!{Colors.END}")
